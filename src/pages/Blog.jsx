  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        setLoading(false); // Set loading to false when the blog is fetched
        console.log(res.data);
      } catch (error) {
        console.log(error);
        setLoading(false); // If there's an error, stop loading as well
      }
    };
    fetchBlog();
  }, [id]);
  ...
  return (
    <div className="font-serif flex space-x-10">
      <div className="space-y-4 lg:w-[50rem] md:w-[30rem] flex-grow rounded-b bg-light">
        {loading ? (
          // Show loading spinner while the blog is being fetched
